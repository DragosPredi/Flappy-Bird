...
(provide (struct-out abilityStruct))
...
(struct abilityStruct (image time pos next))

; Fiecare funcție returneaza o componenta a unei abilități.
(define (get-ability-image ability) (abilityStruct-image ability))
(define (get-ability-time  ability) (abilityStruct-time ability))
(define (get-ability-pos   ability) (abilityStruct-pos ability))
(define (get-ability-next  ability) (abilityStruct-next ability))
...
(define (assignIfNull elem)
  (if (null? (get-ability-pos elem)) (struct-copy abilityStruct elem [pos (random-position POSITION_RANGE)]) elem))
(define (position-abilities abilities)
         (map assignIfNull abilities))
...
(define (compose-abilities L)
	(map compose (map get-ability-next L)))
